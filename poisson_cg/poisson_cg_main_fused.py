#!/usr/bin/env python3
"""
Taichi Poisson + CG — fused SpMV edge kernel (same math as `poisson_cg_main.py`).

Fuses **gather** (Af[e] * p[dst[e]]) and **scatter** (atomic add to Ap[src[e]]) into one
edge loop, matching Kokkos `poisson_cg_fused.cpp` and the fusion rationale in README.md.
"""
...
def write_timing_csv(
    ...
    # Match taichi/swe_main_fused.py naming and CSV schema.
    ms_per_iter = seconds / iterations * 1000.0
    name = f"timing_{arch}_taichi_fused.csv"
    if output_dir:
...
def main() -> None:
    ...
    print(f"[taichi-poisson-cg] nc={nc}, ne={ne} (fused SpMV)")
    ...
    def spmv_fused():
        for i in range(nc):
            Ap[i] = 0.0
        for e in range(ne):
            ti.atomic_add(Ap[src[e]], Af[e] * p[dst[e]])
        for i in range(nc):
            Ap[i] += Ac[i] * p[i]
    ...
    csv_path = write_timing_csv(args.output, "fused", args.arch, iters, seconds)
